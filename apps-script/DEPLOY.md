...
- Save requests compare a content-and-format revision, acquire a script lock, retain a private JSON backup, and submit a single Sheets batch. With unchanged row structure, only edited cells are written. A stale revision is merged when local and remote edits affect different cells. A same-cell conflict or concurrent row insertion, deletion, or reorder is rejected with a precise message. Human edits made directly in Sheets are not locked by Apps Script during the instant they are typed.
